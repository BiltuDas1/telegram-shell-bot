...
urlmsgid="$2"
# Starting
curl --Silent "$fetch/sendMessage?chat_id=${TARGETGROUP}&text=Scanning The Url🔍" --Output "temp/bot.log"
...
if [ "$rescode" == "1" ]; then
  ...
  if [ "$positives" -le "3" ]; then
    curl --Silent "$fetch/sendMessage?chat_id=${TARGETGROUP}&text=The URL is safe.☑️" --Output "temp/bot.log"
    msg_id=$(jq '.result.message_id' "temp/bot.log")
    sleep 5
    curl --Silent "$fetch/deleteMessage?chat_id=${TARGETGROUP}&message_id=$msg_id"
  else
    if [ "$positives" -le "5" ]; then
      ...
      curl --Silent "$fetch/sendMessage?chat_id=${TARGETGROUP}&text=This Message needs additional Review.📝" --Output "temp/bot.log"
      msg_id=$(jq '.result.message_id' "temp/bot.log")
      sleep 10
      curl --Silent "$fetch/deleteMessage?chat_id=${TARGETGROUP}&message_id=$msg_id"
    else
      curl --Silent "$fetch/deleteMessage?chat_id=${TARGETGROUP}&message_id=$urlmsgid"
      curl --Silent "$fetch/sendMessage?chat_id=${TARGETGROUP}&text=The URL is Dangerous.⚠️" --Output "temp/bot.log"
      msg_id=$(jq '.result.message_id' "temp/bot.log")
      sleep 5
      curl --Silent "$fetch/deleteMessage?chat_id=${TARGETGROUP}&message_id=$msg_id"
    fi
